...
void Home::loadBankAccounts() {
    ...
    std::vector<QString> columnNames , values ;
    ...
    while(query.next()) {
        ...
        for(auto i = columnNames.begin() ; i != columnNames.end() ; i++) {
            query.prepare("SELECT " + (*i) + " FROM BankAccounts WHERE CardName = :cardname ") ;
            query.bindValue(":cardname" , crdNum) ; query.exec() ;
            if(query.next()) values.push_back(query.value(0).toString()) ;
        }
        ...
        m_bankAccounts.MList<BankAccount>::pushBack(bnkAccnt) ;
